...
public class SocialGooglePlayHelper implements
	...
	public interface GameHelperListener {
		...
		void onSignInSucceeded(String accountID, String email, String token);
	}
	...
	private static boolean mStaticGetToken = false;
	private static boolean mStaticIsGetTokening = false;
	 ...
	 */
	public SocialGooglePlayHelper(Context activity, int scope) {
		mActivity = (Activity) activity;
		mAppContext = mActivity.getApplicationContext();
		mRequestedClients = scope;
	}
	 ...
	 */
	public void setup(GameHelperListener listener, Activity act) {
		if (mSetupDone) {
			// String error =
			// "GameHelper: you cannot call GameHelper.setup() more than once!";
			// logError(error);
			// throw new IllegalStateException(error);
			return;
		}
	...
	public void onStart(Activity act) {
		...
		if (mConnectOnStart) {
			if (mGoogleApiClient.isConnected()) {
				...
				if (false == mConnecting && false == mStaticIsGetTokening
						&& false == mGoogleApiClient.isConnecting()) {
	...
	public void onActivityResult(int requestCode, int responseCode,
			Intent intent) {
		debugLog(String.format("onActivityResult %d : %d ", requestCode,
				responseCode));
		// /!
		if (requestCode == SocialGooglePlayUtils.USER_RECOVERABLE_AUTH
				&& responseCode == Activity.RESULT_OK) {
			getGooglePlayToken();
		}
		if (requestCode == SocialGooglePlayUtils.RC_SIGN_IN) {
			debugLog("onActivityResult: req="
					+ (requestCode == RC_RESOLVE ? "RC_RESOLVE" : String
							.valueOf(requestCode))
					+ ", resp="
					+ SocialGooglePlayUtils
			...
			if (responseCode == Activity.RESULT_OK) {
				...
				giveUp(new SignInFailureReason(
						mConnectionResult.getErrorCode(), responseCode));
			}
		}
	}
	...
	void notifyListener(boolean success) {
		...
		if (mListener != null) {
			if (success) {
				if (true == mStaticGetToken) {
					mActivity.runOnUiThread(new Runnable() {
						public void run() {
							mListener.onSignInSucceeded(mAccountID, mEmail,
									mToken);
						}
					});
					return;
				}
				mStaticGetToken = true;
				getGooglePlayToken();
			} else {
	...
	public void connect() {
		...
		mInvitation = null;
		mTurnBasedMatch = null;
		if (false == mGoogleApiClient.isConnecting()) {
			mGoogleApiClient.connect();
		}
	}
	...
	public void onConnectionFailed(ConnectionResult result) {
		...
		debugLog("   - code: "
				+ SocialGooglePlayUtils.errorCodeToString(mConnectionResult
						.getErrorCode()));
	...
	void giveUp(SignInFailureReason reason) {
		...
		if (reason.mActivityResultCode == GamesActivityResultCodes.RESULT_APP_MISCONFIGURED) {
			...
			SocialGooglePlayUtils.printMisconfiguredDebugInfo(mAppContext);
		}
	...
	public static void showFailureDialog(Activity activity, int actResp,
		...
		case GamesActivityResultCodes.RESULT_APP_MISCONFIGURED:
			errorDialog = makeSimpleDialog(activity,
					SocialGooglePlayUtils.getString(activity,
							SocialGooglePlayUtils.R_APP_MISCONFIGURED));
			break;
		case GamesActivityResultCodes.RESULT_SIGN_IN_FAILED:
			errorDialog = makeSimpleDialog(activity,
					SocialGooglePlayUtils.getString(activity,
							SocialGooglePlayUtils.R_SIGN_IN_FAILED));
			break;
		case GamesActivityResultCodes.RESULT_LICENSE_FAILED:
			errorDialog = makeSimpleDialog(activity,
					SocialGooglePlayUtils.getString(activity,
							SocialGooglePlayUtils.R_LICENSE_FAILED));
			break;
		default:
			...
			if (errorDialog == null) {
				...
				errorDialog = makeSimpleDialog(
						activity,
						SocialGooglePlayUtils.getString(activity,
								SocialGooglePlayUtils.R_UNKNOWN_ERROR)
								+ " "
								+ SocialGooglePlayUtils
	...
	public static class SignInFailureReason {
		...
		public String errorDesc() {
			return SocialGooglePlayUtils.errorCodeToString(mServiceErrorCode)
					+ "("
					+ SocialGooglePlayUtils
							.activityResponseCodeToString(mActivityResultCode)
					+ ")";
	...
	protected void getGooglePlayToken() {
		class TokenThread implements Runnable {
			private Activity mActivity;
			private String email;

			public TokenThread(Activity activity, String email) {
			...
			public void run() {
				try {
					debugLog("start token ");
					final String accountID = GoogleAuthUtil.getAccountId(
							this.mActivity, this.email);
					Bundle b = new Bundle();
					final String ACTIVITIES_LOGIN = "http://schemas.google.com/AddActivity";
					b.putString(GoogleAuthUtil.KEY_REQUEST_VISIBLE_ACTIVITIES,
							ACTIVITIES_LOGIN);
					final String token = GoogleAuthUtil
							.getToken(
									this.mActivity,
									this.email,
									"oauth2:https://www.googleapis.com/auth/plus.login email",
									b);
					...
					if (intent != null) {
						mActivity.startActivityForResult(intent,
								SocialGooglePlayHelper.USER_RECOVERABLE_AUTH);
					}
					mStaticGetToken = false;
				} catch (Exception e) {
					e.printStackTrace();
				...
				mStaticIsGetTokening = false;
			}
		}
		if (false == mStaticIsGetTokening) {
			TokenThread getToken = new TokenThread(mActivity, getEmail());
			Thread thread = new Thread(getToken);
			thread.start();
			mStaticIsGetTokening = true;
		}
	}

	public String getEmail() {

		final String accountName = Plus.AccountApi
					...
					if (isSignedIn()) {
						Games.Leaderboards
								.loadCurrentPlayerLeaderboardScore(
										mGoogleApiClient, leaderboardID,
										...
										new ResultCallback<Leaderboards.LoadPlayerScoreResult>() {
											...
											public void onResult(
												...
												if (loadPlayerScoreResult != null) {
													if (GamesStatusCodes.STATUS_OK == loadPlayerScoreResult
															.getStatus()
															.getStatusCode()) {
														Games.Leaderboards
																.submitScore(
	...
	public void showLeaderboard(final String leaderboardID) {
		PluginWrapper.runOnMainThread(new Runnable() {
			public void run() {
				try {
					if (isSignedIn()) {
						final int REQUEST_LEADERBORAD = 2048;
						if (null == leaderboardID
							...
							mActivity.startActivityForResult(Games.Leaderboards
									.getLeaderboardIntent(mGoogleApiClient,
											leaderboardID), REQUEST_LEADERBORAD);
						}
					}
				} catch (Exception e) {
					signOut();
	...
	public void showAchievements() {
		PluginWrapper.runOnMainThread(new Runnable() {
			public void run() {
				try {
					if (isSignedIn()) {
						final int REQUEST_ACHIEVEMENTS = 1024;
						mActivity.startActivityForResult(Games.Achievements
	...
	public void unlockAchievement(final Hashtable<String, String> achInfo) {
		// TODO Auto-generated method stub
		PluginWrapper.runOnMainThread(new Runnable() {
			public void run() {
				try {
					if (isSignedIn()) {
						String achieveID = achInfo.get("id");
